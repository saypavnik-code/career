...
export default function CareerDashboard() {
  ...
  async function requestAi(action: AiAction, artifact: Record<string, unknown>, competencyIds: string[] = []) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || 'Не удалось получить подсказку')
      return data
    } catch (error) {
...
function TodayView({ quickText, onQuickText, onSubmit, captures, activeIdeas, coachNote, onClassify, onOpenIdea, onNewIdea, onOpenWin, onView }: {
  ...
  return <div className={styles.pageStack}>
    <section className={`${styles.heroCard} ${styles.calmHero}`}>
      ...
      <form className={`${styles.quickCapture} ${styles.quickThought}`} onSubmit={onSubmit}><textarea value={quickText} onChange={(event) => onQuickText(event.target.value)} placeholder="Например: новый угол с собственными данными может заинтересовать бизнес-СМИ…" aria-label="Быстрая заметка" /><button type="submit">Записать</button></form>
    ...
    {captures.length > 0 && <section className={styles.panel}><div className={styles.sectionHeader}><div><span className={styles.eyebrow}>Последние записи</span><h3>Разобрать позже</h3></div></div><div className={styles.captureList}>{captures.map((capture) => {
      ...
      return <article className={styles.captureCard} key={capture.id}><p>{capture.text}</p>{capture.status === 'unclassified' ? <div className={styles.suggestionBar}><span>{suggestion.reason}</span><div><button type="button" onClick={() => onClassify(capture, 'idea')}>Похоже на идею</button><button type="button" onClick={() => onClassify(capture, 'win')}>Похоже на результат</button><button type="button" onClick={() => onClassify(capture, 'note')}>Оставить заметкой</button></div></div> : <span className={styles.softLabel}>{capture.status === 'converted' ? 'Перенесено' : 'Заметка'}</span>}</article>
    })}</div></section>}

    <div className={styles.twoColumn}>
      <section className={styles.panel}><div className={styles.sectionHeader}><div><span className={styles.eyebrow}>Продолжить</span><h3>Идеи в работе</h3></div><button className={styles.secondaryButton} type="button" onClick={() => onView('ideas')}>Все идеи</button></div><div className={styles.cardList}>{activeIdeas.length ? activeIdeas.map((idea) => <article className={styles.ideaCompact} key={idea.id}><div><span className={styles.statusBadge} data-status={idea.status}>{statusLabels[idea.status]}</span><h4>{idea.title}</h4><p>{idea.nextStep || 'Следующий шаг пока не определён'}</p></div><button type="button" onClick={() => onOpenIdea(idea)}>Открыть</button></article>) : <EmptyState title="Пока нет идей" text="Запишите свободную мысль или создайте идею сразу." action="Создать идею" onAction={onNewIdea} />}</div></section>
      <section className={styles.promptCard}><span className={styles.eyebrow}>Подсказка Эскады</span><h3>{coachNote?.title || 'Сохраняйте доказательства по ходу работы'}</h3><p>{coachNote?.text || 'Ссылка, артефакт, отзыв или принятое решение делают будущий отчёт убедительнее.'}</p>{coachNote?.ideaId && <button type="button" onClick={() => onView('ideas')}>Открыть идеи</button>}</section>
    </div>
  </div>
}
...
function ReportsView({ wins, ideas, selectedWinIds, selectedIdeaIds, periodStart, periodEnd, reportType, reportText, reports, guidance, busy, error, onPeriodStart, onPeriodEnd, onReportType, onToggleWin, onToggleIdea, onSelectAll, onGenerate, onReview, onReportText, onSave }: {
  ...
  return <div className={styles.pageStack}><section className={styles.pageIntro}><div><span className={styles.eyebrow}>Вы контролируете содержание</span><h2>Соберите отчёт</h2><p>Выберите период, тип документа и только те достижения, которые должны войти в черновик.</p></div></section><section className={styles.reportTypeGrid}>{(Object.entries(reportTypeLabels) as Array<[ReportType, string]>).map(([value, label]) => <button type="button" key={value} className={reportType === value ? styles.reportTypeActive : styles.reportTypeCard} onClick={() => onReportType(value)}><strong>{label}</strong></button>)}</section><div className={styles.reportLayout}><section className={styles.panel}><div className={styles.dateGrid}><label>С<input type="date" value={periodStart} onChange={(event) => onPeriodStart(event.target.value)} /></label><label>По<input type="date" value={periodEnd} onChange={(event) => onPeriodEnd(event.target.value)} /></label></div><div className={styles.sectionHeader}><div><span className={styles.eyebrow}>Достижения</span><h3>Выберите wins</h3></div><button className={styles.textButton} type="button" onClick={onSelectAll}>Выбрать все</button></div><div className={styles.reportWins}>{wins.map((win) => <label className={styles.reportWin} key={win.id}><input type="checkbox" checked={selectedWinIds.includes(win.id)} onChange={() => onToggleWin(win.id)} /><div><strong>{win.title}</strong><span>{formatDate(win.date)}</span>{winGapHints(win as unknown as Record<string, unknown>).slice(0, 1).map((hint) => <small key={hint}>{hint}</small>)}</div></label>)}</div><details className={styles.optionalBlock}><summary>Добавить идеи в работе — необязательно</summary><div className={styles.reportWins}>{ideas.map((idea) => <label className={styles.reportWin} key={idea.id}><input type="checkbox" checked={selectedIdeaIds.includes(idea.id)} onChange={() => onToggleIdea(idea.id)} /><div><strong>{idea.title}</strong><span>{idea.nextStep || 'Без следующего шага'}</span></div></label>)}</div></details><button className={styles.primaryButton} type="button" disabled={!selectedWinIds.length || busy === 'report_draft'} onClick={() => void onGenerate()}>{busy === 'report_draft' ? 'Эскада собирает…' : 'Собрать черновик с Эскадой'}</button>{error && <p className={styles.aiError}>{error}</p>}</section><section className={styles.panel}><div className={styles.sectionHeader}><div><span className={styles.eyebrow}>Редактор</span><h3>{reportTypeLabels[reportType]}</h3></div></div><textarea className={styles.reportEditor} value={reportText} onChange={(event) => onReportText(event.target.value)} placeholder="После выбора wins Эскада соберёт редактируемый черновик." /><div className={styles.buttonRow}><button className={styles.secondaryButton} type="button" disabled={!reportText || busy === 'report_review'} onClick={() => void onReview()}>{busy === 'report_review' ? 'Проверяем…' : 'Проверить по шкале'}</button><button className={styles.primaryButton} type="button" disabled={!reportText} onClick={onSave}>Сохранить версию</button></div></section></div>{guidance && <AiGuidancePanel guidance={guidance} />}{reports.length > 0 && <section className={styles.savedReports}><div className={styles.sectionHeader}><div><span className={styles.eyebrow}>История</span><h3>Сохранённые версии</h3></div></div>{reports.slice(0, 5).map((report) => <article key={report.id}><strong>{report.title}</strong><span>{formatDate(report.createdAt.slice(0, 10))}</span></article>)}</section>}</div>
}
